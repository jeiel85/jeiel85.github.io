...
1. **Google Play Store 실시간 스크래핑 (`google-play-scraper` 활용)**
...
2. **프리미엄 & 반응형 UI 디자인 (Vanilla CSS & JS)**
...
3. **GitHub Actions를 통한 100% 완전 자동화 배포**
...
포트폴리오의 모든 데이터(개발자 정보 및 프로젝트 목록)는 루트 디렉토리의 **`apps.json`** 파일 하나로 관리됩니다. 이 파일만 수정하여 커밋하면 GitHub Actions가 나머지를 자동으로 빌드하고 배포합니다.
